...
async def f():
    try:
        ...
        if r.status_code==200:i("Config fetched successfully");return{k:r.json()[k]for k in['endpoints','model','system_prompts','prompt_templates']if k in r.json()}
...
async def m(a):
    try:
        ...
        c.info("Test Results")
        [c.info(f"Endpoint {w}! {n}: {tt}s")for n,tt,r in q if(w:="OK"if r=="OK"else"Fail")]
