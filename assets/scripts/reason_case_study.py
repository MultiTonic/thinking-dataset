...
async def main(a):
    try:
        global dr;dr=await sd(a)
        l(f"Config URL: {a.config}");l(f"Split: {a.split}");l(f"Output: {a.output}")
        l(f"Run ID: {dr['r']}");l(f"Data dir: {dr['d']}");c=await g(a.config)
        if not c:l("Failed to load config");return
        k,m=await v(c,a.split)
        if not k:l(f"Invalid configuration: {m}");return
        l("Config successfully loaded");s=c["splits"][a.split]
        if a.src:l(f"Overriding source from '{c.get('src','')}' to '{a.src}'");c['src']=a.src
        src=c.get('src')
        if not src:l("No source dataset specified in config or arguments");return
        l(f"Source: {src}");l(f"Using split: {a.split} ({s})")
        l(f"System prompt length: {len(c['systems'][s])}");l(f"Prompt template length: {len(c['prompts'][s])}")
        l(f"Column mappings: query={c['columns']['query']}, response={c['columns']['response']}, think={c['columns']['think']}")
        d=await ld(src,a.split)
        if not d:l("Failed to load dataset");return
        q=c['columns']['query'];r=c['columns']['response'];t=c['columns']['think'];ds=d[a.split]
